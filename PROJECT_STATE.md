...
The coordinated Android visual-system follow-up is now in staged implementation. The accepted Soft Kinetic baseline uses a warm off-white light canvas, a deep sage hierarchy, a restrained dark palette, low elevation, consistent typography and rounding, a passive three-position primary-page indicator, and shared lower-thumb controls without adding persistent Calendar/Today/Training navigation. The first production slice landed on 2026-08-27 across Today, Calendar, and the Training library. Xiaomi 14 light/dark verification covered the compact Today card, Calendar week-distance feedback and conditional return-to-today action, Training scene selection, data-preserving installation, and the absence of exposed delete color around routine cards. The second production slice landed the same day across routine preview, active timer, and post-session completion: preview groups logical actions; the active surface uses a compact routine/progress/total-remaining topline, dominant current-action countdown, thin progress indicator, scan-first cues and warnings, the next logical action rather than internal prepare/switch/rest fragments, and integrated lower controls; completion clearly separates the already-saved immutable timer fact from the optional formal training record. Xiaomi 14 acceptance covered dark preview, dark running portrait, light paused portrait, and landscape recreation without losing the active timer or returning to Today. The third production slice establishes a shared secondary-space header and low-elevation card language across daily review, planning, data, and settings. Planning now names its two real jobs explicitly, stored import timestamps are localized, body trends use the shared card hierarchy, and settings entries expose consistent icon containers and direction affordances. The theme now defines the complete Material surface-container ladder so sheets no longer fall back to an unrelated default tint. Settings, AI configuration, and backup sheets scroll at large font sizes. The fourth slice unifies loading, empty, offline, warning, failure, and success presentation and adds the previously missing visible sync-conflict resolution entry. The 2026-08-28 fidelity correction then aligns the real Today, Data, Plan, Settings, and full daily-review surfaces directly with the accepted prototype: the Today plan card is compact and artwork-backed, body status is a four-part scan row, secondary utilities remain phone-scoped sheets, planning keeps exactly two explicit jobs, settings is a divided action list, and DeepSeek uses the replaceable whale identity. Completed-review text now uses valid non-negative Compose padding; the real completed result can open and scroll without the state-specific crash found during device QA. The existing information architecture, accessibility contrast gate, record/timer ownership, day priority, and phase-1 scope remain unchanged. Final motion polish remains a later slice of the same system rather than unrelated page-by-page styling.

## Current Release State and Next Step

The fourth visual-system production slice landed on 2026-08-27 across Today, Calendar details, Training library, daily review, planning, data, cloud connection, AI settings, and sync conflicts. One reusable state panel now gives loading, empty, offline, warning, explicit failure, and success states consistent icons, hierarchy, containers, actions, and polite accessibility announcements. Daily-review `RETRY` remains server-authoritative and exposes no manual resubmission; manual retry appears only after explicit `FAILED`. Settings now surfaces existing Room conflicts without exposing internal IDs and allows the user to choose local or cloud per item through the repository's existing resolution semantics. Xiaomi 14 verification covered light/dark conflict presentation, 1.5x-font scrolling and action reachability, data-preserving same-signature installation, unchanged conflict count after read-only inspection, and restoration of theme, font, and rotation settings. No conflict, AI job, plan, timer fact, or formal record was created or resolved during validation. Motion remains the final planned visual-system slice; phase-2 scope and all existing ownership semantics remain unchanged.

The 2026-08-29 structural follow-up corrects four coherence defects found in Xiaomi 14 review without changing navigation or data semantics. Landscape timer now keeps the upcoming logical action beneath the current-action hero in the wider left task column and reserves the right column for cues and warnings. Today keeps the same four-slot body-status skeleton before and after recording, with missing fields explicitly shown as `未记录`. Plan and Settings now share one bordered secondary-action-card component. Data opens directly at its content height with the complete trend chart visible, removing the partial-sheet/fixed-height interaction that required a manual pull and then exposed blank space. Unit tests, Lint, Android-test compilation, same-certificate installation, and reference/native screenshot comparison passed; no user business data or device rotation preference changed.

The 2026-08-30 Plan follow-up fixes the shared negative-padding crash that closed both Import and Recent Review, and sizes those stable subpages to a content-fitting intermediate sheet height. Their complete initial controls are visible without dragging, while longer validation results can still scroll. The same-certificate Xiaomi 14 update preserved the original first-install identity; both paths stayed foreground with no new crash, and verification did not generate a review or modify business data.

The 2026-08-30 bottom-sheet follow-up replaces that page-specific sizing with one application-wide contract. Every phone bottom sheet now measures short content naturally, keeps only modest breathing space, caps content at two thirds of the available screen, skips the unstable intermediate detent, and scrolls internally only when content exceeds the cap. Data, Plan, Settings, Calendar details, check-in editors, daily review, connectivity/configuration tools, and post-workout editing use the same host. Xiaomi 14 verification covered natural-height and capped surfaces, including immediate complete Data-chart visibility; the crash buffer stayed empty and no business action was submitted.

The 2026-08-30 Today/Data interaction follow-up integrates morning measurements into a compact summary card and gives empty, running, failed, and complete coach-review states one stable provider-aware card anatomy. Data now treats horizontal motion exclusively as metric navigation: its four tabs and chart pages share a continuously animated pager state, while incidental sideways movement and scrim taps cannot dismiss the sheet. A clear horizontal handle remains visible, but an explicit downward gesture may begin anywhere inside Data and uses native continuous finger-following sheet motion; system back remains the non-gesture close path. Xiaomi 14 verification covered linked weight/body-fat/muscle transitions, both dismissal paths, same-certificate data-preserving installation, and an empty crash buffer; no review or business mutation was triggered.

The 2026-08-30 Today body-detail correction removes the redundant normal/missing pain sentence below the four-part body scan; only actual pain produces a dedicated warning. The former floating pre-workout text link is now a complete secondary action row inside the morning-measurement card, with a compact fallback card when no measurements exist. Xiaomi 14 verification confirmed the revised hierarchy after a same-certificate update without invoking any save or review-generation action.

The follow-up containment correction makes body status one complete Today card: its heading/action, scan values, conditional pain warning, morning measurements, and pre-workout action share one outer surface with flat divided subsections instead of uncontained text or nested cards. Same-certificate Xiaomi 14 verification confirmed the result without changing business data.

The Today action-grammar correction preserves the full-width filled day action while unifying every card-local edit, detail, generation, and retry entry as one compact tonal text-plus-arrow button. This removes the previous mixture of arrowless pills, bare text links, and boxed arrow actions. Same-certificate Xiaomi 14 inspection confirmed consistent dimensions and hierarchy without invoking an action.

The 2026-08-30 Training-library interaction correction replaces each scene's vertical routine list with one nested horizontal routine pager. Each page carries the routine summary, a short action preview, an explicit detail affordance, and an explicit delete action with the existing confirmation semantics; the bottom scene selector remains unchanged. The nested pager owns routine-to-routine movement, but its leading boundary hands a continued drag toward Today back to the primary pager, while Android system back continues to return directly to Today. No routine classification, deletion semantics, timer ownership, or plan selection behavior changed.

The 2026-08-30 planning-entry correction removes an illegal negative Compose padding from the shared planning subpage header. The defect crashed `MainActivity` before either the plan-patch inbox or recent-review workspace could render, so both visible Plan actions appeared to return to the launcher. Both actions retain the accepted provider-neutral clipboard semantics; no planning entity, import validation, weekly snapshot, schema, ownership, or navigation contract changed. The Android regression path now opens Import, returns through the shared subpage header, and opens Recent Review in sequence.

The 2026-08-27 landscape density correction gives the active timer a dedicated wide-screen composition instead of compressing the portrait Scaffold. The compact topline now begins immediately below the system-icon safety band, the action/countdown and guidance columns consume the available height, and previous/pause-or-resume/next/stop move into a right-side vertical control rail. Xiaomi 14 verification covered running and paused states, all five accessibility control labels, Activity recreation, checkpoint cleanup, and restoration of the device rotation setting. Portrait layout and timer/session ownership are unchanged.

The 2026-08-27 primary-pager chrome correction replaces the earlier per-page passive dots with one shared pager-driven indicator. Its selected capsule follows the continuous drag offset between Calendar, Today, and Training, while the three page-local fixed overlays are removed. The correction also removes duplicated Scaffold/status-bar insets and reduces page-local top spacing so headers share one compact safety band without moving beneath system icons. Xiaomi 14 endpoint and mid-drag captures verified all three pages and an in-between indicator position. Timer preview and active timer continue to hide primary-page chrome.

The 2026-08-27 secondary-surface correction completes the next planned visual slice without changing domain behavior. Daily review, plan collaboration, 30-day body trends, and settings now use one hierarchy for headings, cards, and supporting text. The Material light/dark surface-container tokens are complete rather than partially inheriting framework defaults, and long settings sheets remain reachable at 1.5x font scale through native scrolling. Unit tests cover localized plan-import timestamps; Android unit and lint gates pass; Xiaomi 14 retained its application data, theme schedule, font scale, and rotation settings.

The 2026-08-28 prototype-fidelity correction completes the next reviewable secondary-surface pass. Today now uses the accepted compact artwork-backed plan card, a four-part body-status scan row, and the provider-specific coach identity. Data, Plan, Settings, and the complete daily review are independent phone-scoped sheets over Today; Data presents one of four body trends at a time, Plan exposes only import and recent-review generation, Settings uses a low-elevation divided list, and the review uses flat scan-first sections instead of a single oversized card. Side-by-side reference/native comparisons for all five surfaces pass in `design-qa.md`. Device validation also caught and fixed a completed-review-only crash caused by negative start padding; the short-conclusion regression case compiles and the real completed review opens and scrolls without a new AndroidRuntime exception. The same-certificate install preserved the 2026-08-12 first-install identity and all application data. Motion remains the next visual-system slice.

The 2026-08-28 sync-role correction fixes conflict resolution for native timer facts. Android now partitions a mixed outbox by authoritative owner: Shenk-owned records use the Shenk credential, while `timer_sessions` use the timer credential; pulls continue through the Shenk read role. Choosing “保留本机” therefore requeues the selected fact against the observed remote revision without immediately recreating an authorization conflict. If the timer credential is unavailable, the fact remains retryable locally rather than being sent under the wrong role. The correction does not resolve, rewrite, or delete any existing user conflict automatically. All 44 isolated data-sync tests passed on Xiaomi 14, including mixed-role routing and local-resolution revision handling. The same-certificate APK then passed a data-preserving install and 1893 ms cold launch while retaining the original first-install identity; the test package was removed and device auto-rotate remained off.

Package 6 passed its automated gates and Xiaomi 14 acceptance on 2026-08-08. Its accepted phase-1 advanced-AI transport is provider-neutral clipboard copy/paste: Android copies the normalized weekly review package, and accepts only a pasted `coach_plan_patch` that passes whole-patch validation, preview, and confirmation. Device acceptance covered strict add/apply, routine-only empty-array no-op, whole-patch rejection, explicit-delete confirmation, latest-only undo, offline outbox recovery, weekly feedback copy, and cleanup. Apply and undo now release their busy state before showing result snackbars. Package 7 daily AI review passed its automated gates and Xiaomi 14 acceptance on 2026-08-09. Package 8 then began as the final phase-1 delivery package. P8.0 baseline audit and P8.1 reproducible release foundation passed on 2026-08-10, establishing centralized versioning, external release signing, and CI release validation. P8.2 authenticated foreground update passed automated, deployed-Worker, data-preserving install, APK integrity/signature, and no-release cold-start gates on Xiaomi 14 on 2026-08-12. P8.3 local reminders and HyperOS guidance passed automated and Xiaomi 14 gates on 2026-08-12; the reminder surface now exposes actual notification and battery state, public repair routes, and honest HyperOS delivery limits without a permanent service or remote push. Device diagnostics found the app notification switch disabled by HyperOS, and the app reports that state without changing it. P8.4 performance and stability passed automated and Xiaomi 14 gates on 2026-08-12, including cold start, primary transitions, calendar scrolling, one-hour virtual timer execution, a realistic 13-month projection, and a full sync batch. A 2026-08-13 interaction follow-up first retained all three primary pages, then moved their initial local-data composition and hidden pre-draw behind a bounded AndroidX system SplashScreen gate. Forty continuous Xiaomi 14 transitions completed at 1.41% janky, p95 18 ms and p99 29 ms; the focused post-splash first gesture produced no Choreographer skipped-frame burst. The debug tradeoff was an approximately 2.1-2.4 second fully prepared time-to-interactive; the signed P8.8 release candidate later reported 3015 ms Android 16 cold-launch `WaitTime`. Timer recovery no longer depends on Training composition, calendar resolution is indexed per Room emission, and cloud synchronization is coordinated through WorkManager. P8.5 accessibility and theme validation passed on 2026-08-15. Automated contrast gates enforce 4.5:1 for light/dark text tokens and calendar accents; fixed control heights now expand with text; primary navigation and routine deletion expose accessibility actions; and the splash skips its fade when system animators are disabled. Xiaomi 14 accepted all three primary pages at 1.5x font size, fully visible 48 dp touch targets at 480 dpi, Android accessibility-node labels/actions, and zero-animation launch/navigation. The device has no TalkBack package, so spoken-output testing was not claimed; acceptance used the platform accessibility contract TalkBack consumes. Font and animation settings were restored. P8.6 backup, migration, and security regression passed on 2026-08-22. Settings now exposes secret-free SAF business backup; restore validates complete v1/v2 envelopes before a transaction and safely merges without replacing existing local state; restored Shenk-owned records replay through outbox while timer facts remain local; encrypted migration profiles enforce the Web-compatible PBKDF2/AES-GCM shape and roll back partial local replacement; and test access codes are generated transiently. All 61 repository tests, Package 8 foundation gates, and 42 isolated Xiaomi 14 tests passed, including a synthetic ContentResolver JSON round trip. The data-preserving install kept the original first-install time and Room database, the debug cold start completed in 2035 ms, and the temporary backup URI and test package were removed. P8.7 full regression passed on 2026-08-22: all 61 cross-repository tests, Android Debug/Release/lint/JVM gates, 42 isolated synthetic-data device tests, and three read-only production-package checks passed. The data-preserving install retained the original first-install identity and `shenk-native.db`; Calendar-Today-Training accessibility navigation and Settings -> Data backup reachability passed; the final cold launch was 2110 ms; ordinary UI remained portrait-locked without changing the device auto-rotate setting; system font/animation state and notification state were unchanged; and all test packages plus synthetic backup rows were removed. P8.8 signed release candidate passed on 2026-08-22. Version `1.0.0-rc.1` / `versionCode 11` was built from committed revision `756b4a3` through the mandatory external-signing gate and archived locally with its checksum, source revision, certificate digest, byte size, and known-good forward-rollback revision; no signing secret entered Git or the release record. Xiaomi 14 accepted the same-certificate data-preserving update while retaining its original first-install identity and existing Today/Calendar content. Calendar–Today–Training navigation, system back, and Settings -> Data backup passed on the signed build; Android 16 reported 3015 ms cold-launch `WaitTime`; auto-rotate remained off; font and animation scales remained `1.0`; and no test package, active Shenk notification, temporary key copy, or temporary signing ACL remained. Package 8 and native Android phase 1 are complete at `9 / 9`.

Phase 1 is complete. The whole-product visual-system follow-up has moved from audit/prototype into staged native implementation. The brand and adaptive-icon foundation, Calendar–Today–Training primary slice, native timer preview/active/completion slice, daily-review/planning/data/settings secondary slice, and unified non-content states are established. The next reviewable slice is the final motion polish: transitions should clarify spatial continuity and state change while respecting reduced motion and the accepted pager performance gate. This work must preserve the accepted information architecture, accessibility, performance, data ownership, and day-priority semantics, and it does not authorize phase-2 widgets, wearables, or animation assets.

...
Package 5 is ready but not started: Native timer and routine library. It must consume explicit routine `scene` and `role`, keep `timer_sessions` separate from formal `training_logs`, work offline from the routine cache, and preserve the timer module's exclusive write ownership. Plan inbox and AI review remain outside Package 5.

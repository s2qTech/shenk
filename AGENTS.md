...
- 正式 Android 第一期 Packages 0-8 已全部通过门禁，进度为 `9 / 9`。签名私有发布候选 `1.0.0-rc.1` 已在 Xiaomi 14 完成保留数据覆盖安装与关键路径验收；后续产品工作从已记录的整体视觉系统审计与规划开始，不得顺带引入 phase 2 功能。
...
1. 日历主显示优先级固定为：正式实际记录 > 当前有效正式计划 > AI 本地建议 > 确定性离线兜底建议。
2. 计划调整解析为当天唯一有效计划；普通 UI 不并列展示计划和调整历史。
3. 缺失数据保持缺失，不自动记为休息，也不把未填写字段推断为正常。
4. 高级 AI 计划高于本地建议；用户实际执行和身体反馈是最终事实。
5. 日常兼容 AI 可以综合事实生成简评，并在策略边界内把本地建议写入 `daily_reviews`；不得修改正式计划、计划调整、目标、策略或 routine 实体。
...
14. 第一期高级 AI 协作仅允许复制规范化复盘资料和粘贴 `coach_plan_patch`；不得调用系统分享、绑定特定厂商或绕过校验。后期 API、MCP 或 Skill 只能替换传输适配层，必须复用相同规划快照、patch、权限、确认和审计语义。

...
## 2026-08-30 Routine preview return hierarchy
...
## 2026-08-31 final motion implementation

The implementation introduces one shared native motion contract without changing the accepted screen composition or behavior. Existing programmatic Calendar/Today/Training, Data metric, and Training scene changes use the same `240 ms` adjacent-page timing and `280 ms` two-page timing. Training Library → Preview → Active uses a small directional shared-axis transition, Today coach-review content changes within its existing card, and Calendar's existing week-distance and return-to-today controls use the shared `180–240 ms` appearance contract. A retained preview snapshot prevents the outgoing Preview surface from observing a cleared timer request while the Back transition completes. Gesture-driven pagers remain native and finger-coupled; disabling Android system animators makes the added transitions resolve immediately.

JVM tests, Android-test compilation, Lint, and debug APK assembly passed. Xiaomi 14 accepted the same-certificate `adb install -r` update while preserving the 2026-08-12 first-install identity and existing local data. Real-device checks covered Calendar ↔ Today ↔ Training paging, Data metric label taps and chart swipes, Training scene label changes and the settled-Home boundary return, Library → Preview → Library, Calendar's conditional return-to-today control, and the existing screen compositions. Android zero-animation mode completed all added transitions immediately; the original animator, transition, and window scales were restored to `1.0`. Auto-rotate remained off and the AndroidRuntime crash buffer stayed empty.

The focused pager performance run reset `gfxinfo` after startup and exercised 40 Calendar/Today/Training transitions, mixing finger-driven paging with programmatic system-back returns. Android reported 4,042 rendered frames, one janky frame (`0.02%`), p95 `18 ms`, and p99 `28 ms`, retaining the accepted performance gate. The first updated cold launch completed in `2206 ms`. No business record, plan, AI job, timer fact, data contract, or sync path was changed. Temporary screenshots and the accessibility dump contained real on-device values and were deleted after inspection.

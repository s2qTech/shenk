...
## 2026-08-30 Training routine paging

The Training library now treats routines inside one selected scene as adjacent horizontal pages instead of a vertical stack. A page uses the available canvas for title, duration, action count, a short action preview, and an explicit detail affordance; the scene switcher stays in the lower thumb zone. The former card-level swipe-to-delete gesture was removed because it competed with routine paging. Deletion remains available through a visible icon, the existing confirmation dialog, and the accessibility custom action.

The inner pager observes but does not consume the leading-boundary gesture. On the routine page nearest Today, a continued drag toward Today invokes the outer primary transition; Android system back already follows the same destination. A focused Compose regression covers forward/back routine paging and the additional leading-boundary gesture, and the existing navigation regression covers system back from Training. Debug Kotlin, Android-test Kotlin, JVM unit tests, Lint, and debug assembly passed. A same-certificate `adb install -r` succeeded and retained the original 2026-08-12 first-install identity; the device disconnected before the final expanded-card screenshot and boundary-gesture pass, so those last manual observations are not claimed here. No routine was opened, deleted, or modified during the connected-device inspection.

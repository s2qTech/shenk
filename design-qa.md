...
Data remains a capped phone-scoped sheet, but its dismissal and metric navigation are now separated. A tap on the scrim and incidental horizontal movement cannot dismiss it. The sheet closes only through Android system back or a deliberate downward drag from its handle. Weight, body fat, muscle, and waist are pages in one horizontal pager; direct tab selection and chart swipes share the same pager state, while the selected capsule follows the continuous page offset.

Automated verification passed JVM unit tests, Lint, debug Android-test compilation, and signed debug assembly. The same-certificate APK updated Xiaomi 14 without clearing application data. Real-device verification confirmed the integrated morning-measurement and empty-review cards, complete first-open chart visibility, ignored scrim taps, ignored short horizontal movement, linked weight/body-fat/muscle swipe transitions, deliberate handle dismissal, system-back dismissal, a live application process, and an empty Android crash buffer. No review was generated and no business data was saved or modified during verification.

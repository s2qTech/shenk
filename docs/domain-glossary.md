...
| AI 本地建议 | AI local suggestion | 日常 AI 依据状态、测量、睡眠、近期训练、目标和策略生成的当日建议；保存在 `daily_reviews` 内，只在没有正式计划时参与当天指导 |
| 确定性离线兜底 | deterministic offline fallback | 没有实际记录、正式计划和可用 AI 本地建议时，由本地固定规则产生的通用方向；不替代 AI 计划 |
...
| 每日简评 | `daily_reviews` | 兼容 AI 综合当天与近期事实生成的结论、专业判断、行动、风险和证据；可附带当日本地建议，但不具备修改正式计划、策略或 routine 的权限 |
...
- 正式计划不等于 AI 本地建议，AI 本地建议也不等于确定性离线兜底。
- 计划调整历史不等于第二份并列计划。

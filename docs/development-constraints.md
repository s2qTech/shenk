...
- 训练页的居家、健走、恢复、外出是同一内层横向分页的四个场景，不是把单个训练方案改成分页；底部场景标签与内容必须共享连续分页状态。各场景内部方案仍使用纵向列表。训练页稳定显示时，横向手势只能由内层场景分页持有，外层主分页必须锁定；只有从已稳定的居家边界开始的一次独立拖动才可显式切回 Today。禁止把健走到居家的快速滑动所剩速度继续交给外层，系统返回则直接回 Today。
- 训练方案卡片轻点进入、长按才请求删除；普通表面不得常驻删除按钮。长按只能打开原有确认弹窗，不能直接删除，并须保留等价的无障碍删除动作。
- 进入训练方案预览后必须暂时锁定外层主分页；普通横滑不得直接离开方案。系统边缘返回先退出预览回训练列表，再次返回才回 Today。预览返回与开始训练必须共用底部对齐的按钮操作栏，不保留悬空的顶部文字返回链接。
...
- 今日身体状态必须作为一张完整主卡呈现，标题、记录/修改、四格扫描、真实疼痛警示、晨间测量和训练前状态共用一个外层容器并以分隔线组织；禁止恢复成无容器正文，也禁止把晨间测量做成卡中卡。四格扫描结果已经表达正常与缺失状态，不得在其后重复一行同义小字；只有真实疼痛才增加明确警示。训练前状态不得作为悬空文字链接。当天正式训练、休息或跳过记录确认后，身体状态停止编辑；已有训练前状态只读展示，未记录的训练前状态不再提供补录入口。
- 今日页所有进入编辑、详情、生成或重试流程的卡片内操作统一使用紧凑浅色按钮，并包含文字和右箭头；日级主操作使用全宽实色按钮和右箭头。禁止在同一层级混用无箭头胶囊、裸文字箭头链接和有框箭头按钮。
- 数据趋势卡属于持续查看界面：横向翻页、轻微斜向移动和点击遮罩不得关闭。顶部必须保留清晰可见的横条提示，但明确的向下手势可从卡片任意位置发起；卡片必须随手指连续位移并按原生底部卡片规则回弹或关闭，禁止到阈值后直接消失。系统返回是无手势替代入口。四项指标的标签与内容必须由同一分页状态驱动，并提供点击标签的无手势替代入口。
- Android 现有界面的过渡统一使用 `180 / 240 / 280 ms` 三档共享动效 token 和同一空间缓动；手指驱动的主分页、数据分页和训练分类分页必须持续跟手，不得叠加整页缩放或透明度层。程序化跳转和稳定状态切换可使用共享 token，但不得借动效修改布局、文案、导航层级、手势结果或业务状态。系统关闭动画时必须立即完成切换。

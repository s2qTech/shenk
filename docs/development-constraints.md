...
- 今日身体状态必须作为一张完整主卡呈现，标题、记录/修改、四格扫描、真实疼痛警示、晨间测量和训练前状态共用一个外层容器并以分隔线组织；禁止恢复成无容器正文，也禁止把晨间测量做成卡中卡。四格扫描结果已经表达正常与缺失状态，不得在其后重复一行同义小字；只有真实疼痛才增加明确警示。训练前状态不得作为悬空文字链接。

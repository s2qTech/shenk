...
- 高级 AI patch 可更新正式计划和 routine；日常兼容 AI 只可写 `daily_reviews`，并可在其中附带当日本地建议，不得写计划、调整、目标、策略或 routine。
- 当存在当前有效正式计划时，日常兼容 AI 的本地建议必须被服务端剥离，并在客户端解析时继续受优先级保护。
...
- 日历显示优先级必须是：实际记录 > 当前有效计划 > AI 本地建议 > 确定性离线兜底。
...
- 所有底部上升卡片统一按内容自适应，只保留安全区和少量留白；最大内容高度为可用屏幕的三分之二。超过后固定在该上限并滚动卡片内部内容。禁止为单页设置互相冲突的固定高度比例，也禁止短内容被撑成伪全屏。
...
- 不因“现代化”重写已经稳定的产品页面。
- Android 只支持当前自用主设备对应的稳定 Android 平台；不得为假设中的旧机型、旧系统增加兼容分支、降级交互或保守依赖。
- Android 构建使用当前 LTS JDK，`minSdk` 随当前正式支持平台提高，不维持面向公共分发的历史 API 覆盖率。
- JDK 运行版本、编译 toolchain、JVM 字节码目标和 Android API 是独立决策；不得把 AGP 的最低 JDK 要求误当成项目应选版本，也不得把高 JDK 误写成 Android 尚未支持的 class-file target。
- 依赖选择以官方稳定兼容交集为准。只要兼容交集成立就优先新稳定版；预览版或兼容矩阵外组合必须有明确收益、隔离验证和回退方案。

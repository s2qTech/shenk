...
- `SHENK_TOKEN`: write 身刻-owned records such as plans, training logs, body metrics, media metadata, and feedback summaries. `timer_session_links` remains compatible for legacy links and Android `action: "ignored"` acknowledgements; new formal-record flows must use `training_logs.timerSessionId` / `training_logs.timerSessionIds` instead of link-only `linked` or `converted` handling records.

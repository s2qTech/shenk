...
- After that warm-up, the pager retains the complete three-page set even while Calendar or Training is selected. The previous one-page retention radius dropped the opposite edge page, so returning to Today could compose that hidden page during the gesture and produce an occasional long frame.
...
| Three-page retained debug-process memory after cold start and 3.5-second warm-up | total PSS 242,010 KB; total RSS 385,896 KB |
...
The edge-retention instrumentation test compiled as part of the Package 8 gate and asserts both hidden edge pages remain composed while either edge is selected. HyperOS rejected installation of the temporary app instrumentation APK twice with `INSTALL_FAILED_USER_RESTRICTED`, so that focused device test was not reported as executed. The production APK installed successfully, and the direction-specific frame measurements above ran against that installed correction.
...
Reverting the P8.4 commits restores eager adjacent-page composition, edge-page disposal while the opposite edge is selected, eager idle timer platform initialization, repeated per-day guidance scans, and direct startup synchronization. No database rollback, data conversion, or cleanup is required.

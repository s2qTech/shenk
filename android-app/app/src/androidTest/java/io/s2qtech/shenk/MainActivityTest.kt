...
class MainActivityTest {
    ...
    @Test
    fun trainingRoutinesPageHorizontallyAndLeadingBoundaryReturnsToToday() {
        val routines = listOf(
            testRoutine("pager-first", "方案一"),
            testRoutine("pager-second", "方案二"),
        )
        ...
        composeRule.onNodeWithTag("routine-pager-first").assertIsDisplayed()
        composeRule.onNodeWithTag("training-routine-pager").performTouchInput { swipeLeft() }
        composeRule.waitForIdle()
        composeRule.onNodeWithTag("routine-pager-second").assertIsDisplayed()

        composeRule.onNodeWithTag("training-routine-pager").performTouchInput { swipeRight() }
        composeRule.waitForIdle()
        composeRule.onNodeWithTag("routine-pager-first").assertIsDisplayed()
        composeRule.onNodeWithTag("training-routine-pager").performTouchInput { swipeRight() }
        composeRule.waitForIdle()
    ...
    private fun testRoutine(id: String, title: String) = decodeRoutineTemplate(
        SharedRecord.create(
            entity = "routine_templates",
            id = id,
            data = buildJsonObject {
